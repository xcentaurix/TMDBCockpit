...
class ScreenMain(Picture, Json, Screen, HelpableScreen):
    ...
    def getData(self, menu_selection, text, ident, page, callback):
        ...
        reactor.callFromThread(callback, totalpages, result)  # pylint: disable=no-member
    ...
    def gotData(self, totalpages, result, text=""):
        ...
        if result:
            if self.direction_up:
                self["list"].setIndex(len(self.result) - 1)
            else:
    ...
    def up(self):
        logger.info("SelectionIndex: %s, len(self.result): %s",
                    self["list"].getSelectedIndex(), len(self.result))
    ...
    def down(self):
        logger.info("SelectionIndex: %s, len(self.result): %s",
                    self["list"].getSelectedIndex(), len(self.result))
        if self["list"].getSelectedIndex() == len(self.result) - 1:
            self.prevBouquet()
        else:
            self["list"].down()
