...
def js_to_json(code):
    ...
    def fix_kv(m):
        ...
        if v in {'undefined', 'void 0'}:
            return 'null'
        if v[0] in {'/', '!'} or v.startswith(('/*', '//', '!')) or v == ',':
            return ''
...
_OPERATOR_RE = '|'.join([re.escape(x[0]) for x in _OPERATORS + _LOG_OPERATORS])
...
class JSInterpreter:
    ...
    def _named_object(self, namespace, obj):
        ...
        name = f'__youtube_jsinterp_obj{self.__named_object_counter}'
        namespace[name] = obj
        return name

    @classmethod
    def _separate(cls, expr, delim=',', max_split=None):
        if not expr:
            return
    ...
    def _separate_at_paren(cls, expr, delim=None):
        ...
        if len(separated) < 2:
            raise RuntimeError(f'No terminating paren {delim} in {expr}')
    ...
    def _operator(self, op, left_val, right_expr, local_vars, allow_recursion):
        ...
        try:
            ...
            raise RuntimeError('Failed to evaluate', left_val, op, right_val, e) from e
    ...
    def _index(self, obj, idx):
        ...
        try:
            ...
            raise RuntimeError('Cannot get index', idx, e) from e
    ...
    def interpret_statement(self, stmt, local_vars, allow_recursion=100):
        ...
        if md.get('assign'):
            ...
            if left_val in (None, JSUndefined):
                raise RuntimeError('Cannot index undefined variable', m.group('out'))

            idx = self.interpret_expression(m.group('index'), local_vars, allow_recursion)
            if not isinstance(idx, (int, float)):
                raise RuntimeError(f'List index {idx} must be integer')
            idx = int(idx)
        ...
        for op, _ in self._all_operators():
            ...
            if op in {'+', '-'}:
                ...
                for dm_op in ('*', '%', '/', '**'):
                    bodmas = tuple(self._separate(left_val, dm_op))
        ...
        if md.get('attribute'):
            ...
            def assertion(cndn, msg):
                ...
                if not cndn:
                    raise RuntimeError(f'{member} {msg}')
            ...
            def eval_method(variable, member):
                ...
                if obj is str:
                    if member == 'fromCharCode':
                        assertion(argvals, ARG_MSG)
                        return ''.join([chr(x) for x in argvals])
                ...
                if member == 'splice':
                    assertion(isinstance(obj, list), LIST_MSG)
                    assertion(argvals, ARG_MSG)
                    index, how_many = [int(x) for x in (argvals + [len(obj)])[:2]]
                    if index < 0:
                        index += len(obj)
                    add_items = argvals[2:]
                    res = []
    ...
    def extract_object(self, objname):
        ...
        for obj_m in re.finditer(
            rf'''(?xs)
                {_NAME_RE}\s*\.\s*{re.escape(objname)}|{re.escape(objname)}\s*=\s*{{\s*
    ...
    def build_arglist(cls, arg_text):
        ...
        def valid_arg(y):
            y = y.strip()
            if not y:
                raise RuntimeError(f'Missing arg in "{arg_text}"')
            return y
