...
def custom_urlopen(url, timeout=5):
    """
    ...
    """
    custom_urlopen.response = None
    custom_urlopen.error = None

    def open_url(url, timeout):
        try:
            custom_urlopen.response = urlopen(url, timeout=timeout)  # pylint: disable=consider-using-with
        except Exception as e:
            custom_urlopen.error = e

    t = Thread(target=open_url, args=(url, timeout), daemon=True)
    t.start()
    t.join(timeout + 1)
    if custom_urlopen.error:
        raise custom_urlopen.error
    return custom_urlopen.response
...
class YouTubeVideoUrl():
    ...
    def __init__(self):
        ...
        self._create_priority_formats()

    def _create_priority_formats(self):
        """Build priority format list based on user's resolution config"""
        itag = config.plugins.tmdbcockpit.yttrailer_best_resolution.value
        video_formats = (
        ...
        for video_format in video_formats:
            result = video_format + result
            if video_format[0] == itag:
                break
        YouTubeVideoUrl.priority_video_format = result
    ...
    def _download_webpage(self, url, data=None, headers=None):
        ...
        if data or headers:
            url = Request(url, data=data, headers=headers)
            url.get_method = lambda: 'POST'

        try:
            urlh = custom_urlopen(url, timeout=5)
        except URLError as e:  # pragma: no cover
            raise RuntimeError(e.reason) from e
    ...
    def _extract_n_function_name(jscode):
        ...
        if not idx:
            return func_name
        if int(idx) == 0:
            real_nfunc = re.search(
                fr'var {re.escape(func_name)}\s*=\s*\[([a-zA-Z_$][\w$]*)\];',
                jscode
            )
            if real_nfunc:
    ...
    def _extract_player_info(self):
        ...
        if res:
            player_id = re.search(r'player\\?/([0-9a-fA-F]{8})\\?/', res)
            if player_id:
    ...
    def _load_player(self, player_id):
        if player_id and player_id not in self._player_cache:
            self._player_cache[player_id] = self._download_webpage(
                f'https://www.youtube.com/s/player/{player_id}/player_ias.vflset/en_US/base.js'
            )
    ...
    def _unthrottle_url(self, url, player_id):
        n_param = re.search(r'&n=(.+?)&', url).group(1)
        n_id = f"nsig_{player_id}_{'_'.join(str(len(p)) for p in n_param.split('.'))}"
        ...
        if self.nsig_cache[1]:
            print(f'[YouTubeVideoUrl] Decrypted nsig {self.nsig_cache[0]} => {self.nsig_cache[1]}')
    ...
    def _decrypt_signature(self, s, player_id):
        ...
        s_id = f"sig_{player_id}_{'_'.join(str(len(p)) for p in s.split('.'))}"
    ...
    def _parse_m3u8_attributes(attrib):
        return {key: val[1:-1] if val.startswith('"') else val for (key, val) in re.findall(r'(?P<key>[A-Z0-9-]+)=(?P<val>"[^"]+"|[^",]+)(?:,|$)', attrib)}
    ...
    def _extract_from_m3u8(self, manifest_url):
        ...
        for line in manifest.splitlines():
            if audio_urls and line.startswith('#EXT-X-STREAM-INF:'):
                ...
                itag = re.search(r'/sgovp/[^/]+itag%3D(\d+?)/', line) or re.search(r'/itag/(\d+?)/', line)
                if itag:
        ...
        for fmt in streaming_formats:
            ...
            if itag == our_format and self._not_in_fmt(fmt, itag):
                ...
                if not url and 'signatureCipher' in fmt:
                    sc = parse_qs(fmt.get('signatureCipher', ''))
                    sig = self._decrypt_signature(sc['s'][0], player_id)
                    if sig:
                        url = f"{sc['url'][0]}&{sc['sp'][0] if 'sp' in sc else 'signature'}={sig}"
                if url:
                    if '&n=' in url:
    ...
    def _extract_fmt_video_format(self, streaming_formats, player_id):
        ...
        for our_format in YouTubeVideoUrl.priority_video_format:
    ...
    def _extract_web_response(self, video_id):
        ...
        if webpage:
            player_response = re.search(r'ytInitialPlayerResponse\s*=\s*({[^>]*})\s*;\s*(?:var\s+meta|</script|\n)', webpage)
            if player_response:
                try:
                    return json.loads(player_response.group(1)), self._extract_player_info()
    ...
    def _extract_player_response(self, video_id, yt_auth, client):
        ...
        if client == 5:
            VERSION = '19.29.1'
            USER_AGENT = f'com.google.ios.youtube/{VERSION} (iPhone16,2; U; CPU iOS 17_5_1 like Mac OS X;)'
            data['context'] = {
                'client': {
                    'hl': 'en',
            ...
            VERSION = '19.29.37'
            USER_AGENT = f'com.google.android.youtube/{VERSION} (Linux; U; Android 11) gzip'
            data['context'] = {
                'client': {
                    'hl': 'en',
                    'clientVersion': VERSION,
            ...
            headers['User-Agent'] = USER_AGENT
        try:
            return json.loads(self._download_webpage(url, data, headers)), player_id
        except ValueError:  # pragma: no cover
    ...
    def _real_extract(self, video_id, yt_auth):
        ...
        if config.plugins.tmdbcockpit.yttrailer_useDashMP4.value:
            self.use_dash_mp4 = ()
        else:
        ...
        if YouTubeVideoUrl.priority_video_format and YouTubeVideoUrl.priority_video_format[0] != config.plugins.tmdbcockpit.yttrailer_best_resolution.value:
            self._create_priority_formats()
